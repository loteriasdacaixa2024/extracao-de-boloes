...
MAPA_ESPECIAIS_RAPIDO = {
    'DSP': 'dia-de-sorte-plus',
    'QSJ': 'quina-sao-joao',
    'LTI': 'lotofacil-independencia',
    'MSV': 'mega-sena-virada',
    'MS3': 'mega-sena-da-virada-3',
}
...
def _coletar_modalidade_pre_extracao() -> Optional[object]:
    ...
    # Especiais: QSJ, DSP, etc.
    if resp in MAPA_ESPECIAIS_RAPIDO:
        mod = resolver_modalidade_menu(MAPA_ESPECIAIS_RAPIDO[resp])
        if mod:
...
def _imprimir_resumo_final(
    ...
    print(f'  Registros únicos   : {len(hashes)} (hash_bolao)')
...
def salvar_parcial(boloes, arquivo_base, pagina: int = 0):
    ...
    if not boloes:
        if os.path.isfile(path):
            ...
            _out(f'  [SAVE] Pág {pagina or "?"}: 0 reg. nesta pág. — mantém {os.path.basename(path)} ({kb:.1f} KB).')
        else:
            _out(f'  [SAVE] Pág {pagina or "?"}: 0 reg. — nada gravado ainda.')
        return path

    final, novos, anteriores = salvar_json_continuacao(path, boloes)
    if final:
        kb = os.path.getsize(path) / 1024
        extra = f' | +{novos} novo(s)' if anteriores else ''
        if pagina:
            _out(
                f'  💾 SALVO pág {pagina}: {len(final)} reg. total{extra} | '
                f'{kb:.1f} KB | {os.path.basename(path)}'
            )
        else:
            _out(f'  💾 SALVO: {path} ({len(final)} registro(s), {kb:.1f} KB)')
    return path
...
def _recuperar_boloes_das_capturas(cfg, parser_slug, mod_slug, arquivo_base, rodada=1):
    ...
    if not boloes and brutos:
        _out(f'  [RECUPERO] {len(brutos)} bolão(ões) na API, mas 0 passaram no filtro.')

    if boloes:
        path = os.path.join(PASTA_JSON, f'{arquivo_base}.json')
        final, novos, anteriores = salvar_json_continuacao(path, boloes)
        mod_b = extrair_modalidade_de_boloes(boloes)
        consolidar_sessao(PASTA_JSON, extrair_concurso_de_boloes(boloes), mod_b.slug if mod_b else mod_slug, boloes)
...
def _capturar_pagina_atual(
    ...
    # ── Callback: grava no JSON a cada bloco de detalhes capturado ──────────
    def _salvar_tempo_real(boloes_parciais):
        if arquivo_base and boloes_parciais:
            filt = _boloes_do_filtro(boloes_parciais, cfg)
            filt, _ = _filtrar_boloes_modalidade(filt, mod_esperada)
            if filt:
                for b in filt:
                    b['pagina'] = pagina
                    ...
                    boloes.append(b)
                salvar_parcial(filt, arquivo_base, pagina)
    ...
    if not novos and n_esperado == 0:
        from boloes_api_caixa import coletar_boloes_das_capturas
        novos = coletar_boloes_das_capturas(driver, hashes, print, cfg, parser_slug, filtrar_dezenas=True)
        if novos:
            print(f'  [RECUPERO] {len(novos)} bolão(ões) via lista API.')
    ...
    if n_esperado and len(novos) < n_esperado:
        print(f'  [AVISO] Página incompleta: {len(novos)}/{n_esperado} bolões.')

    descartados = antes_filtro - len(novos)
    if descartados:
        painel['descartados_loterica'] += descartados
        print(f'  [FILTRO] {descartados} descartado(s) — fora do filtro {cfg.termo}')
...
def _loop_extracao_paginas(
    ...
    painel_extra=None, mod_esperada=None,
):
    boloes: list = []
    hashes: set = set()
    ...
    painel['arquivo_base'] = arquivo_base   # ← garante que o painel tem o path correto
    if painel_extra:
    ...
    painel['arquivo_base'] = arquivo_base   # ← atualiza após possível renomeação
    ...
    print(f'  Filtro ativo: {lot_txt} | dezenas: {dez}')
    print(f'  JSON desta rodada: json-boloes/{arquivo_base}.json  (atualizado em tempo real)')
    if mod_esperada:
    ...
    while True:
        ...
        page_boloes = _boloes_do_filtro([b for b in boloes if b.get('pagina') == pagina], cfg)
        ...
        if subset:
            novo_base = _renomear_json_sessao(arquivo_base, subset, mod_esperada)
            if novo_base != arquivo_base:
                arquivo_base = novo_base
                painel['arquivo_base'] = arquivo_base

        # Salva conjunto completo acumulado (deduplicado)
        salvar_parcial(subset, arquivo_base, pagina)
        if subset:
            _consolidar_e_resumir(subset, mod_esperada)

        if voce_encerra:
            print(f'\n  Página {pagina} concluída ({len(page_boloes)} reg.). Total: {len(subset)} reg.')
    ...
    if not subset_final:
        recuperados = _recuperar_boloes_das_capturas(cfg, parser_slug, mod_slug, arquivo_base, rodada_filtro)
        if recuperados:
