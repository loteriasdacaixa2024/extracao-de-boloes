...
def _aviso_sonoro_login_ok(logger: logging.Logger) -> None:
    """Beep curto após login confirmado no portal (Windows)."""
    try:
        import winsound
        winsound.MessageBeep(winsound.MB_OK)
        winsound.Beep(880, 220)
        winsound.Beep(1175, 280)
    except Exception as exc:
        logger.debug("Aviso sonoro de login indisponível: %s", exc)
        try:
...
<<<<<<< HEAD
    if _parece_logado_no_portal(driver) or _focar_se_ja_logado(driver, logger):
        ...
        logger.warning(
            "Login não confirmado no portal — aviso sonoro de sucesso não emitido."
        )
        print(
            "\n  [LOGIN] Não confirmei o login automaticamente. "
            "Se o Edge já estiver logado, digite SIM no terminal em seguida.",
            flush=True,
        )


def _focar_se_ja_logado(driver: WebDriver, logger: logging.Logger) -> bool:
    """Varre abas/janelas; se achar portal logado, foca nela e retorna True."""
    try:
